...
// LUT definition here (one and only one definition)
// const uint16_t LUT[40][40] = {
...
void mem_throt_period_timer_callback(irqid_t int_id) {
    ...
    // Reset the counter to 0 so it measures anew from this point forward.
    events_cntr_set(cpu()->vcpu->vm->mem_throt.counter_id, 0);

    // Re-enable the counter to start counting again.
    events_cntr_enable(cpu()->vcpu->vm->mem_throt.counter_id);

    // Reschedule this interrupt for 100 ms (or whatever period_counts means).
...
void mem_throt_timer_init(irq_handler_t handler) {
    ...
    cpu()->vcpu->vm->mem_throt.period_counts = timer_init(10000);
}
...
void mem_throt_init() {
    console_printk ("mem_throt_init on cpu %d\n", cpu()->id);
    mem_throt_events_init(inst_retired);
